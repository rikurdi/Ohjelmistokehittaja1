...
namespace OpiskelijaRekisteri
{
    class Opiskelija
    {
        ...
        {
            MySqlCommand komento = new MySqlCommand("SELECT Oid, etunimi, sukunimi, puhelin, sähköposti FROM yhteystiedot", yhteys.otaYhteys());
        ...
        {
            MySqlCommand komento = new MySqlCommand();
            String paivityskysely = "UPDATE `yhteystiedot` SET `Etunimi` = @enm," + "`Sukunimi` = @snm," +
                "`puhelin` = @puh," + "`sähköposti` = @eml," + "`opiskelijanumero` = @ono" + "WHERE oid = @oid";
